...
main :: IO ()
main = defaultMain $ testGroup "AoC3"
    ...
    , testCase "part1 example" $ do
        ...
        case parse parser "test_input" input of
            ...
            Right parsed -> assertEqual "part1" 357 (part1 parsed)
    , testCase "part2 example" $ do
        input <- TIO.readFile "test_input"
        case parse parser "test_input" input of
            Left err -> assertFailure (show err)
            Right parsed -> assertEqual "part2" 3121910778619 (part2 parsed)
    , testCase "part1 final" $ do
        input <- TIO.readFile "input"
        case parse parser "input" input of
            Left err -> assertFailure (show err)
            Right parsed -> assertEqual "part1" 16973 (part1 parsed)
    , testCase "part2 final" $ do
        input <- TIO.readFile "input"
        case parse parser "input" input of
            Left err -> assertFailure (show err)
            Right parsed -> assertEqual "part2" 168027167146027 (part2 parsed)
    ]
