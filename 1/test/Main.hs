...
main :: IO ()
main = defaultMain $ testGroup "AoC1"
    ...
    , testCase "part2 example" $ do
        ...
        case parse parser "test_input" input of
            Left err -> assertFailure (show err)
            Right parsed -> assertEqual "part2" 6 (part2 parsed)
    , testCase "part1 final" $ do
        input <- TIO.readFile "input"
        case parse parser "input" input of
            Left err -> assertFailure (show err)
            Right parsed -> assertEqual "part1" 992 (part1 parsed)
    , testCase "part2 final" $ do
        input <- TIO.readFile "input"
        case parse parser "input" input of
            Left err -> assertFailure (show err)
            Right parsed -> assertEqual "part2" 6133 (part2 parsed)
    ]
